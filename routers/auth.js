...
Auth.post('/api', async (req, res) => {
    ...
    if (auth.hasOwnProperty('login') && auth.hasOwnProperty('password')) {
        ...
        for (let index = 0; index < array.length; index++) {
            const element = array[index];
            try {
                let path = (await (await db).task.getTask(element.idtask)).path;
                if (path.includes("api")) {
                    array_.push(path);
                    if (!demoPath.includes(array_[index].slice(0,array_[index].lastIndexOf("/")))) {
                        demoPath.push(array_[index].slice(0,array_[index].lastIndexOf("/")));
                    }
                }
            } catch (error) {
                console.error(element);
        ...
        if (user[0] && pas_flag) {
            ...
            if (user[0]) {
                ...
                return res.header('x-web-token', token).status(200).json(
                    {info:'hisobga kirish mufaqiyatli.'}
                );
                // return res
                //     .cookie("x-web-token", token, {
                //         httpOnly: true,
